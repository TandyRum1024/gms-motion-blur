...
vec2 rot2D (vec2 uv, vec2 texsize, float ang, float zoom)
{
    /// Calc rotated UV
...
void main()
{
    ...
    for (float i = 0.0; i <= 1.0; i += stepSize)
    {
        ...
        // dither up, if we have to
            ...
            // float offzoom = zoomStr * (i + noiseOffset * ditherFactor);
            baseuv = rot2D(v_vTexcoord, v_vTexsize, offang, 1.0);
        #else
            baseuv = rot2D(v_vTexcoord, v_vTexsize, i * -uVelocity.w * uStrength.z, 1.0);
        #endif
